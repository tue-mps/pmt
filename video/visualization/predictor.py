...
from visualizer import TrackVisualizer, fast_visualize_frames_gpu
...
def _mask_iou(mask_a, mask_b):
    """Compute IoU between two lists of per-frame masks."""
    intersection = 0
    union = 0
    for ma, mb in zip(mask_a, mask_b):
        a = np.asarray(ma).astype(bool)
        b = np.asarray(mb).astype(bool)
        intersection += (a & b).sum()
        union += (a | b).sum()
    return intersection / max(union, 1)


def _deduplicate_instances(pred_masks, pred_labels, pred_scores, pred_ids, iou_thresh=0.5):
    """Remove duplicate instances that share high mask IoU, keeping the higher-scoring one."""
    n = len(pred_scores)
    if n <= 1:
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            if not keep[j]:
                continue
            if _mask_iou(pred_masks[i], pred_masks[j]) > iou_thresh:
                ...
                if pred_scores[i] >= pred_scores[j]:
                    ...
                    keep[i] = False
                    break
    pred_masks  = [pred_masks[k]  for k in range(n) if keep[k]]
    pred_labels = [pred_labels[k] for k in range(n) if keep[k]]
    pred_scores = [pred_scores[k] for k in range(n) if keep[k]]
    if pred_ids is not None:
        pred_ids = [pred_ids[k] for k in range(n) if keep[k]]
    return pred_masks, pred_labels, pred_scores, pred_ids
...
def _get_objects_from_outputs(outputs):
    def _get_objects_from_vis_outputs(outputs):
        ...
        # merge duplicate instances with high mask overlap
        pred_masks_, pred_labels_, pred_scores_, pred_ids_ = _deduplicate_instances(
            pred_masks_, pred_labels_, pred_scores_, pred_ids_
        )
    ...
    def _get_objects_from_vps_outputs(outputs):
        ...
        raw_ids = outputs['pred_ids']
        # Convert tensor IDs to Python ints so dict lookups in id_memories
        # work by value (tensors hash by identity, causing new IDs every frame).
        pred_ids = [int(x) if hasattr(x, 'item') else int(x) for x in raw_ids]
...
class VisualizationDemo(object):
    ...
    def run_on_video(self, frames):
        ...
        total_vis_output = fast_visualize_frames_gpu(
            frames, pred_masks, pred_labels, pred_ids, self.id_memories,
    ...
    def _get_class_names(self):
        """Return combined thing + stuff class name list from metadata."""
        thing = self.metadata.get("thing_classes", None)
        stuff = self.metadata.get("stuff_classes", None)
        if stuff is None:
            return thing
        if thing is None:
            return stuff
...
class VideoPredictor(DefaultPredictor):
    ...
    def __init__(self, cfg):
        ...
        if 'model' in weight.keys():
            weight = weight['model']
        result = self.model.load_state_dict(weight, strict=False)
        if result.missing_keys:
            print("[Checkpoint] Missing keys (in model but not in checkpoint):", result.missing_keys)
        if result.unexpected_keys:
            print("[Checkpoint] Unexpected keys (in checkpoint but not in model):", result.unexpected_keys)
